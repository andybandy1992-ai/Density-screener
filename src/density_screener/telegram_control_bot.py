...
class TelegramControlBot:
    ...
    def __init__(self, config: TelegramConfig, controls: RuntimeControlStore) -> None:
        self._config = config
    ...
    async def _handle_message(self, message: dict[str, Any]) -> None:
        ...
        if text.startswith("/panel") or text.startswith("/start") or text.startswith("/status"):
        ...
        if pending.kind == "set_threshold":
            ...
            if value is None:
                await self._send_text(chat_id, "Пришлите число в USD, например: `75000`.", parse_mode="Markdown")
                return
            snapshot = self._controls.set_min_notional(pending.market_type, value)
            self._pending_actions.pop(pending_key, None)
            await self._send_text(
                chat_id,
                self._format_threshold_confirmation(pending.market_type, snapshot),
            )
            await self._send_panel(chat_id)
            return
        ...
        if pending.kind == "add_blacklist":
            ...
            await self._send_text(
                chat_id,
                f"Rule added: {snapshot.blacklist_terms[-1]}",
            )
            await self._send_panel(chat_id)
            return
    ...
    async def _handle_callback_query(self, query: dict[str, Any]) -> None:
        ...
        if data.startswith("threshold:"):
            ...
            if action == "custom":
                ...
                await self._send_text(
                    chat_id,
                    self._format_threshold_prompt(market_type),
                )
                return
            delta = float(action)
